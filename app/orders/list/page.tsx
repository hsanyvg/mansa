    ...
    const printContent = `
      ...
      <head>
        ...
        <style>
          ...
          ${size === '100x100' ? `
            .receipt > div { margin-top: 1px !important; margin-bottom: 1px !important; }
            .middle-box { min-height: 140px !important; }
            .middle-box-right .qr-code { width: 55px !important; height: 55px !important; }
            .bottom-barcode-row svg { height: 35px !important; }
            .notes-box { flex-grow: 1 !important; min-height: 25px !important; margin-top: 2px !important; margin-bottom: 2px !important; font-size: 8pt !important; }
            table td { padding: 1px 0 !important; font-size: 8pt !important; }
          
            .top-banner-row { min-height: 42px !important; }
            .top-banner-black { width: 100px !important; font-size: 10pt !important; }
            .middle-box > div { padding: 2px 6px !important; }
            .middle-box { margin-bottom: 4px !important; }
            .middle-box > div > div:first-child { margin-bottom: 5px !important; }
            .store-info-row { margin-top: 2px !important; }
            .store-info-row > div:first-child { font-size: 10pt !important; }
            .store-info-row .qr-code { width: 30px !important; height: 30px !important; }
            table { border-spacing: 0 1px !important; }
            .receipt { padding-bottom: 0 !important; }
            .bottom-barcode-row { margin-top: 2px !important; padding-top: 2px !important; margin-bottom: 0 !important; }` : ''}
        </style>
        ...
        ${ordersToPrint.map((order, index) => {
          ...
          return `
            <div class="receipt">
              ...
              <div class="top-banner-row" style="display: flex; border: 1px solid #000; min-height: 55px; position: relative; align-items: stretch;">
                <div class="top-banner-black" style="width: 140px; background: #000; color: #fff; display: flex; align-items: center; justify-content: center; font-size: 11pt; font-weight: bold; clip-path: polygon(25px 0, 100% 0, 100% 100%, 25px 100%, 0 50%); margin-left: -1px;">
                  من المركز
                </div>
                  ...
                  {visibleColumns.status && (
                    <td>
                      ...
                      {order.paymentStatus === 'settled' ? (
                         <div style={{ fontSize: '0.85rem', color: '#10b981', width: '100%', textAlign: 'center', marginTop: '2px', fontWeight: 'bold' }}>
                           (تم المحاسبة)
                         </div>
                      ) : (
                         <div style={{ fontSize: '0.85rem', color: '#ef4444', width: '100%', textAlign: 'center', marginTop: '2px', fontWeight: 'bold' }}>
                           (لم تتم المحاسبة)
                         </div>
                      )}
                      {order.returnBatchId && (
                      ...
                      {order.deliveryStatus && (
                        <div style={{
                          ...
                          تحديث {order.shippingCompany || 'الشحن'}:
                          <br />
                          <span style={{ color: '#fff' }}>{order.deliveryStatus}</span>
                        </div>
                      )}
                    </div>
                  </td>
