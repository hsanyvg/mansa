...
export default function ExpenseTagsPage() {
  const [categories, setCategories] = useState<ExpenseTag[]>([]);
  const [inputName, setInputName] = useState('');
  const [loading, setLoading] = useState(true);
  const [toast, setToast] = useState<{ message: string, type: 'success' | 'error' } | null>(null);

  useEffect(() => {
    const categoriesRef = collection(db, 'users', auth.currentUser?.uid || 'anonymous', 'expense_tags');
    
    const unsubscribe = onSnapshot(categoriesRef, async (snapshot) => {
      const catsData = snapshot.docs.map(d => ({ id: d.id, ...d.data() })) as ExpenseTag[];
      
      // If empty, add default categories
      if (snapshot.empty && loading) {
        setLoading(false); // Prevent infinite loop
        const defaults = ['ستكرات'];
        try {
          const batch = writeBatch(db);
          defaults.forEach(name => {
            const newDocRef = doc(collection(db, 'users', auth.currentUser?.uid || 'anonymous', 'expense_tags'));
  ...
  const handleAdd = async () => {
    ...
    try {
      await addDoc(collection(db, 'users', auth.currentUser?.uid || 'anonymous', 'expense_tags'), {
        name: inputName.trim(),
        createdAt: serverTimestamp()
      });
      setInputName('');
      showToastMsg("تم إضافة الوسم بنجاح");
    } catch (error) {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("هل أنت متأكد من حذف هذه الوسم؟")) return;

    try {
      await deleteDoc(doc(db, 'users', auth.currentUser?.uid || 'anonymous', 'expense_tags', id));
      showToastMsg("تم حذف الوسم بنجاح");
    } catch (error) {
  ...
  return (
    <div className={styles.container}>
      ...
      <header className={styles.header}>
        <h1 className={styles.title}>إدارة وسوم المصروفات</h1>
      </header>

      <div className={styles.card}>
        <div className={styles.addCategoryForm}>
          <div className={styles.formGroup}>
            <label className={styles.label}>اسم الوسم الجديدة</label>
            <input 
              type="text" 
