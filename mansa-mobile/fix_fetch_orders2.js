...
const newFetchOrders = `// 2. Fetch Paginated Orders for Active Tab (Hybrid logic to avoid Composite Indexes)
  useEffect(() => {
    ...
    if (range) {
      // If date range is selected, query ONLY by date. NO ORDER BY!
      // Using where + orderBy on the same field sometimes causes implicit index conflicts.
      ...
      // Order By alone without where does not require composite index.
      ordersQuery = fsQuery(ordersQuery, orderBy('date', 'desc'), limit(2000));
    }
    ...
    const unsub = onSnapshot(ordersQuery, (snapshot) => {
      ...
      // 0. Sort newest first locally! (Because we removed server-side orderBy for date ranges)
...
if (regex.test(content)) {
  ...
  console.log("Successfully replaced fetchOrders with hybrid logic 2!");
} else {
