...
function applyAllFixes() {
    ...
    // 3. Add specific_month to getDateRange
    const targetCustom = "case 'custom':";
    const replacementCustom = `case 'specific_month':
        start = new Date(filterYear, filterMonth, 1, 0, 0, 0, 0);
        end = new Date(filterYear, filterMonth + 1, 0, 23, 59, 59, 999);
        break;
      case 'custom':`;
    if (c.includes(targetCustom) && !c.includes("case 'specific_month':")) {
        c = c.replace(targetCustom, replacementCustom);
    }

    // 4. Add the Modal exactly before the final </SafeAreaView>
    const finalSafeAreaRegex = /<\/SafeAreaView>\s*\);\s*\}\s*(const getStyles|export default)/;
    const modalCode = `
      {/* 5. Date Filter Modal */}
    ...
    if (!c.includes('تطبيق شهر وسنة محددين')) {
        c = c.replace(finalSafeAreaRegex, (match, p1) => {
            return modalCode + '\n  );\n}\n\n' + (p1 === 'const getStyles' ? p1 : 'export default');
        });
    }
    ...
    console.log('App.js fully rebuilt!');
}
